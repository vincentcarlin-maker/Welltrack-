...
const NavItem: React.FC<{ 
  ...
  <button 
    onClick={onClick}
    className={`flex flex-col items-center justify-center w-full py-1.5 transition-colors active:scale-90 transform duration-100 ${isActive ? 'text-neon-blue' : 'text-slate-400 hover:text-slate-600'}`}
  >
    {icon}
    <span className="text-[9px] mt-0.5 font-bold uppercase tracking-tighter">{label}</span>
  </button>
);
...
export const Layout: React.FC<LayoutProps> = ({ children, activeView, onChangeView }) => {
  return (
    <div className="flex flex-col h-[100dvh] bg-slate-50 text-slate-900 overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth-mobile pb-20 w-full">
        {children}
      </main>

      {/* Bottom Navigation - Fixed with Safe Area */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 z-50 shadow-[0_-4px_12px_-1px_rgba(0,0,0,0.03)] pb-[env(safe-area-inset-bottom)]">
        <div className="flex justify-around items-center h-[52px] max-w-md mx-auto px-1">
          <NavItem 
          ...
          />
          <NavItem 
            icon={<User size={20} strokeWidth={activeView === ViewState.PROFILE ? 2.5 : 2} />} 
            label="Profil" 
